...
CREATE TABLE IF NOT EXISTS `aircraft` (
  ...
  CONSTRAINT `CONSTRAINT_2` CHECK (`aircraft_capacity` >= 0)
...
CREATE TABLE IF NOT EXISTS `flights` (
  ...
  CONSTRAINT `CONSTRAINT_3` CHECK (`flight_miles` >= 0)
...
CREATE TABLE IF NOT EXISTS `ticketprice` (
 ...
 `luggage_price` float NOT NULL CHECK (`luggage_price` >= 0),
 PRIMARY KEY (`pricecategory`),
 CONSTRAINT `CONSTRAINT_4` CHECK (`business_price` >= 0),
 CONSTRAINT `CONSTRAINT_5` CHECK (`firstclass_price` >= 0 or NULL),
 CONSTRAINT `CONSTRAINT_6` CHECK (`luggage_price` >= 0)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COLLATE=latin1_swedish_ci;
